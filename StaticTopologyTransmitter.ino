...
void GetData(void)
{
  ...
  Serial.println(message[measurementcounter].nodeID);
  message[measurementcounter].tempc = 100*bme.readTemperature();
  message[measurementcounter].humidity = 100*bme.readHumidity();
...
void loop() {
  ...
  //if (now - last_sent >= interval)
  {
    ...
    measurement payload = { this_node, 100*bme.readTemperature(), 100*bme.readHumidity(), readBattery(), millis() };
